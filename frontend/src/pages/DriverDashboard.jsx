...
import { getPendingRides, getDriverRides, acceptRide, startRide, completeRide } from '../services/api';
...
function DriverDashboard() {
  ...
  const statCards = [
    { icon: '📊', value: completedRides.length, label: 'Ukupno vožnji', bg: 'rgba(30,58,138,0.4)', border: 'rgba(59,130,246,0.3)' },
    { icon: '⭐', value: `${user?.rating || '5.0'} ⭐`, label: 'Prosečna ocena', bg: 'rgba(78,63,0,0.4)', border: 'rgba(234,179,8,0.3)' },
    { icon: '💵', value: '0', label: 'Danas (RSD)', bg: 'rgba(6,78,59,0.4)', border: 'rgba(16,185,129,0.3)' },
    { icon: '💰', value: totalEarnings, label: 'Ukupno (RSD)', bg: 'rgba(59,7,100,0.4)', border: 'rgba(139,92,246,0.3)' },
  ];
      ...
      <div style={styles.container}>
        ...
        <div style={styles.headerRow}>
          <div>
            ...
            <p style={styles.pageSubtitle}>{user?.name}</p>
          </div>
        ...
        <div style={styles.toggleCard}>
          ...
          <div onClick={() => setIsAvailable(!isAvailable)} style={isAvailable ? styles.switchOn : styles.switchOff}>
            ...
            {pendingRides.length > 0 ? pendingRides.map(ride => (
              <div key={ride._id} style={styles.rideCard}>
                <div style={styles.rideHeader}>
                  <div>
                    <p style={styles.rideName}>👤 {ride.passenger?.name}</p>
                    <p style={styles.rideTime}>🕐 {new Date(ride.createdAt).toLocaleTimeString('sr-RS', { hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                  <span style={styles.vehicleBadge}>{ride.vehicleType}</span>
                </div>
                ...
                <button
                  ...
                  disabled={!isAvailable || !!activeRide || loading}
                >
                  ✓ Prihvati vožnju
                </button>
              </div>
          ...
          <div>
            {activeRide ? (
              <div style={styles.activeRideCard}>
                <div style={styles.rideHeader}>
                  <div>
                    <p style={styles.rideName}>🧭 {activeRide.passenger?.name}</p>
                ...
                {activeRide.status === 'accepted' ? (
                  <button style={styles.btnBlue} onClick={handleStartRide} disabled={loading}>
                    ▶ Započni vožnju
                  </button>
                ) : (
                  <button style={styles.btnGreen} onClick={handleCompleteRide} disabled={loading}>
                    ✓ Završi vožnju
                  </button>
                )}
              </div>
        ...
        {activeTab === 'history' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {completedRides.length > 0 ? [...completedRides].reverse().map((ride) => (
              <div key={ride._id} style={styles.historyCard}>
                <div style={styles.rideHeader}>
                  <div>
                    <p style={styles.rideName}>✅ {ride.passenger?.name}</p>
                    <p style={styles.rideTime}>{new Date(ride.createdAt).toLocaleDateString('sr-RS')}</p>
                  </div>
...
const styles = {
  ...
  dotOnline: { width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#34d399', display: 'block' },
  dotOffline: { width: '10px', height: '10px', borderRadius: '50%', backgroundColor: '#9ca3af', display: 'block' },
  ...
  switchOn: { width: '52px', height: '28px', backgroundColor: '#10b981', borderRadius: '999px', cursor: 'pointer', position: 'relative' },
  switchOff: { width: '52px', height: '28px', backgroundColor: '#4b5563', borderRadius: '999px', cursor: 'pointer', position: 'relative' },
  switchThumbOn: { position: 'absolute', right: '4px', top: '4px', width: '20px', height: '20px', backgroundColor: '#fff', borderRadius: '50%' },
  switchThumbOff: { position: 'absolute', left: '4px', top: '4px', width: '20px', height: '20px', backgroundColor: '#fff', borderRadius: '50%' },
  tabBar: {
    display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '4px',
    backgroundColor: 'rgba(30,41,59,0.6)', border: '1px solid rgba(255,255,255,0.1)',
    borderRadius: '12px', padding: '4px', marginBottom: '24px',
  },
  tab: { padding: '10px', backgroundColor: 'transparent', color: '#9ca3af', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '500', fontFamily: "'Inter', sans-serif" },
  tabActive: { padding: '10px', backgroundColor: '#059669', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600', fontFamily: "'Inter', sans-serif" },
  rideCard: { backgroundColor: 'rgba(30,41,59,0.6)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '16px', padding: '24px' },
  activeRideCard: { background: 'linear-gradient(135deg, rgba(30,58,138,0.4), rgba(6,78,59,0.4))', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '16px', padding: '24px' },
  historyCard: { backgroundColor: 'rgba(30,41,59,0.6)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '20px' },
  ...
  vehicleBadge: { backgroundColor: 'rgba(16,185,129,0.2)', color: '#6ee7b7', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '999px', padding: '4px 12px', fontSize: '12px', fontWeight: '600' },
  badgeBlue: { backgroundColor: 'rgba(59,130,246,0.2)', color: '#93c5fd', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '999px', padding: '4px 12px', fontSize: '12px', fontWeight: '600' },
  badgeYellow: { backgroundColor: 'rgba(234,179,8,0.2)', color: '#fde047', border: '1px solid rgba(234,179,8,0.3)', borderRadius: '999px', padding: '4px 12px', fontSize: '12px', fontWeight: '600' },
  routeBox: { backgroundColor: 'rgba(15,23,42,0.5)', borderRadius: '12px', padding: '16px', marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '12px' },
  ...
  rideStatBox: { backgroundColor: 'rgba(15,23,42,0.5)', borderRadius: '10px', padding: '12px' },
  ...
  notesBox: { backgroundColor: 'rgba(30,58,138,0.2)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '10px', padding: '12px', marginBottom: '16px' },
  notesLabel: { color: '#93c5fd', fontSize: '11px', margin: '0 0 4px', fontWeight: '600' },
  notesText: { color: '#d1d5db', fontSize: '14px', margin: 0 },
  btnGreen: { width: '100%', padding: '14px', background: 'linear-gradient(to right, #059669, #0d9488)', color: '#fff', fontWeight: '700', fontSize: '15px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontFamily: "'Inter', sans-serif" },
  btnBlue: { width: '100%', padding: '14px', background: 'linear-gradient(to right, #2563eb, #0891b2)', color: '#fff', fontWeight: '700', fontSize: '15px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontFamily: "'Inter', sans-serif" },
  btnDisabled: { width: '100%', padding: '14px', backgroundColor: '#374151', color: '#6b7280', fontWeight: '700', fontSize: '15px', border: 'none', borderRadius: '8px', cursor: 'not-allowed', fontFamily: "'Inter', sans-serif" },
  emptyCard: { backgroundColor: 'rgba(30,41,59,0.6)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '60px 24px', textAlign: 'center' },
