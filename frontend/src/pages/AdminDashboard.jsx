...
import { getAllRides, getVehicles, addVehicle, deleteVehicle, toggleVehicle } from '../services/api';
...
function AdminDashboard() {
  ...
  const handleAddVehicle = async () => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || 'Greška pri dodavanju vozila');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span style={{ backgroundColor: s.bg, color: s.color, padding: '3px 10px', borderRadius: '999px', fontSize: '12px', fontWeight: '600' }}>
        {s.label}
      </span>
    );
  };
  ...
  const statCards = [
    ...
    { icon: '✅', label: 'Završene vožnje', value: completedRides, color: '#4ade80' },
    ...
    <div style={styles.page}>
      ...
      <div style={styles.container}>
        ...
        <div style={styles.tabBar}>
          {['rides', 'vehicles', 'stats'].map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              style={activeTab === tab ? styles.tabActive : styles.tab}
            >
              {{ rides: 'Vožnje', vehicles: 'Vozila', stats: 'Statistika' }[tab]}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'rides' && (
          <div style={styles.card}>
            ...
            {rides.length === 0 ? (
              <p style={styles.emptyText}>Nema vožnji u sistemu</p>
            ) : (
              <div style={styles.tableWrap}>
                {rides.map(ride => (
                  <div key={ride._id} style={styles.rideRow}>
                    <div style={styles.rideRowLeft}>
                      <p style={styles.ridePassenger}>{ride.passenger?.name}</p>
                      <p style={styles.rideDate}>{new Date(ride.createdAt).toLocaleDateString('sr-RS')}</p>
                      ...
                      {ride.driver && (
                        <p style={styles.rideDriver}>Vozač: {ride.driver?.name} • {ride.distance} km</p>
                      )}
                    </div>
          ...
          <div style={styles.card}>
            ...
            {showAddVehicle && (
              <div style={styles.addVehicleForm}>
                ...
                <button style={styles.btnYellow} onClick={handleAddVehicle} disabled={loading}>
            ...
            <div style={styles.tableWrap}>
              ...
              {vehicles.length === 0 ? (
                <p style={styles.emptyText}>Nema vozila u sistemu</p>
              ) : vehicles.map(v => (
                <div key={v._id} style={styles.tableRow}>
                  ...
                  <span style={{ ...styles.td, display: 'flex', gap: '8px' }}>
                    <button style={styles.btnSmallOutline} onClick={() => handleToggleVehicle(v._id)}>
                      {v.isActive ? 'Deaktiviraj' : 'Aktiviraj'}
                    </button>
                    <button style={styles.btnSmallRed} onClick={() => handleDeleteVehicle(v._id)}>🗑️</button>
                  </span>
                </div>
              ))}
            </div>
...
const styles = {
  ...
  statCard: { backgroundColor: 'rgba(30,41,59,0.6)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '20px' },
  ...
  tabActive: { padding: '10px', backgroundColor: '#eab308', color: '#111', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '700', fontFamily: "'Inter', sans-serif" },
  card: { backgroundColor: 'rgba(30,41,59,0.6)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '24px' },
  ...
  tableHeader: { display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.1)' },
  th: { color: '#9ca3af', fontSize: '12px', fontWeight: '600' },
  tableRow: { display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', alignItems: 'center' },
  td: { color: '#e5e7eb', fontSize: '14px' },
  rideRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '16px', backgroundColor: 'rgba(15,23,42,0.4)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)' },
  ...
  addVehicleForm: { backgroundColor: 'rgba(15,23,42,0.5)', borderRadius: '12px', padding: '20px', marginBottom: '20px', border: '1px solid rgba(255,255,255,0.1)' },
  ...
  formInput: { width: '100%', padding: '8px 10px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', backgroundColor: 'rgba(51,65,85,0.5)', color: '#fff', fontSize: '14px', fontFamily: "'Inter', sans-serif", boxSizing: 'border-box' },
  btnYellow: { padding: '10px 20px', backgroundColor: '#eab308', color: '#111', fontWeight: '700', fontSize: '14px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontFamily: "'Inter', sans-serif" },
  btnSmallOutline: { padding: '5px 10px', backgroundColor: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: '#e5e7eb', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontFamily: "'Inter', sans-serif" },
  btnSmallRed: { padding: '5px 10px', backgroundColor: 'rgba(239,68,68,0.2)', border: '1px solid rgba(239,68,68,0.3)', color: '#f87171', borderRadius: '6px', cursor: 'pointer', fontSize: '12px' },
  error: { backgroundColor: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', color: '#f87171', borderRadius: '8px', padding: '10px 12px', fontSize: '13px', marginBottom: '12px' },
  emptyText: { color: '#9ca3af', textAlign: 'center', padding: '20px 0' },
  ...
  statsListRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.1)' },
